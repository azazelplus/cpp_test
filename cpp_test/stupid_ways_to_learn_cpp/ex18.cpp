...
//这是一个Adress类的方法. 简单的打印而已.此处我们用按指传递. 当然也可以按值传递, 只需要把*去掉, ->改为.即可. 但是大多情 况下, 如果函数是只读类型的, 传入一个结构体或类的时候按指传递更高效灵活.(当然也有一定危险性, 一定要保证这个函数是只读.)
...
int main(int argc, char* argv[])
{
        ...
        if (argc > 3)//如果包含了[action params]
                id = atoi(argv[3]);//atoi()函数全称是ASCII to Integer. 将字符串转换为整数. 处理算法: 跳过空白字符. 处理 正负号, 影响最终结果的符号. 把数字字符转换为数字,把它们拼接起来. 遇到非数字字符时,停止. 例如,对字符串"  -  125 7哈哈123", 输出整数-1257. 当然本例中argv[3]只是简单的一个数字char, 所以直接就转换了.
        if(id >= MAX_ROWS)
                die("id太大了.");
