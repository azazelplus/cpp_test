...
const TCHAR* path,	/* Logical drive number to be mounted/unmounted. 这里是一个char*字符串, 要求格式是"0:", "1:",...函数会自动去掉冒号. */
...
FRESULT f_mkfs (
	...
	BYTE sfd,			/* Partitioning rule 分割规则. 0:FDISK(更通用.), 1:SFD(无分区表, 只格式化整个磁盘, 适合小型磁盘或特定工具) */
	UINT au				/* Size of allocation unit in unit of byte or sector.分配单元(簇)大小.`0`表示自由选择 */
)
{
