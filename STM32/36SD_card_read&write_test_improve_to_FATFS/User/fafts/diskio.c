...
#include <string.h>		//使用mumcpy内存拷贝函数
...
{
	DSTATUS status = STA_NOINIT;//默认错误假设为STA_NOINIT 0x01, 未初始化.
	
	switch (pdrv) {
		case ATA:	/* SD CARD */
			status &= ~STA_NOINIT;//变成0x00.
			break;
...
//地址对齐.许多32位MCU（包括STM32）的DMA控制器要求内存地址4字节对齐才能正确/高效工作!		
//如果出现非对齐地址时，需要建立一个 临时对齐缓冲区（scratch）
		  if((DWORD)buff&3)//检查buff指针能不能被4整除. &3和%4是一样的.
			{
				DRESULT res = RES_OK;
				DWORD scratch[SD_BLOCKSIZE / 4];//创建一个uint8数组作为对齐的临时缓冲区

				while (count--) 
				{
					res = disk_read(ATA,(void *)scratch, sector++, 1);//调用自己,递归
