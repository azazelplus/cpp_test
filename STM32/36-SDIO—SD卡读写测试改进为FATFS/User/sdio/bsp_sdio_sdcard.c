...
/**
  ...
  *        - SD_TRANSFER_BUSY: Data transfer is acting
	*				 - SD_TRANSFER_ERROR: 出错
  */
//这个SD_GetStatus()包装了SD_GetState()包装了SD_SendStatus(). 最终实现: 返回SD卡状态(一个SDTransferState枚举类型: OK/BUSY/ERRPR)
//他对SD_GetState()的包装完全只是把枚举名字变了一下而已(从SDCardState类型变成SDTransferState(工程上常用的ERROR/BUSY/OK三态模型).)
//对应关系:
//SD_CARD_TRANSFER对应SD_TRANSFER_OK
//SD_CARD_ERROR对应SD_TRANSFER_ERROR
//SDCardState的所有其他枚举状态(SENDING; PROGRAMMING...)对应SD_TRANSFER_BUSY,总之就是此时SD没准备好transfer呢.
//很合理.
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF)//如果SD卡处于掉电状态就不执行后面的了, 返回errorstatus
  {
...
/**
  ...
  * @retval SD_Error: SD Card Error code.
  */
//这个函数对SD卡发送CMD13命令, 得到R1响应的结果, 然后写到传入的指针里. 同时函数返回是否操作成功.
SD_Error SD_SendStatus(uint32_t *pcardstatus)
{
  SD_Error errorstatus = SD_OK;

  SDIO->ARG = (uint32_t) RCA << 16;//设置命令参数寄存器. RCA(relative card addr,)
  SDIO->CMD = 0x44D;//44D即为CMD13(查询卡状态)命令帧. 我们还记得, SD card的命令帧是48bit的, 此处44D是3个16进制数字, 也就是48bit.
  
  errorstatus = CmdResp1Error(SD_CMD_SEND_STATUS);//这个函数检查R1响应是否有错误
  ...
  *pcardstatus = SDIO->RESP1;//如果响应没错误, RESP1即R1响应值. 把响应值写入pcardstatus.
  return(errorstatus);
}
